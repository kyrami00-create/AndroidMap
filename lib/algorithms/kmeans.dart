...
class _EatingZonesState extends State<EatingZones>
  ...
  void initState() {
    super.initState();
    _cafes = campusCafes
        .map((c) => CafePoint(name: c.name, position: c.position, type: c.type))
        .toList();
  ...
  void _runClustering() async {
    ...
    setState(() { _clustered = true; _animating = false; });
  }

  void _resetClusters() {
    setState(() {
      for (final c in _cafes) { c.clusterId = -1; }
      _clustered = false;
    });
  }
  ...
  Widget _buildControlPanel() {
    return Container(
      ...
      child: Row(
        children: [
          const Text('K =', style: TextStyle(color: Color(0xFF9A9AB0), fontSize: 13)),
          const SizedBox(width: 4),
          Text('$_kValue', style: const TextStyle(
              color: Color(0xFFB47CFF), fontWeight: FontWeight.bold, fontSize: 16)),
          Expanded(
            child: Slider(
              ...
              onChanged: _clustered ? null : (v) => setState(() => _kValue = v.toInt()),
            ),
          ),
          if (_animating)
            const SizedBox(
              width: 18, height: 18,
              child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFFB47CFF)),
            )
          else
            _ClusterButton(
          ...
          markers: _cafes.map((cafe) {
            ...
            return Marker(
              ...
              child: AnimatedBuilder(
                ...
                builder: (_, __) {
                  final scale = _animating ? 0.85 + _pulseCtrl.value * 0.15 : 1.0;
                  return Column(
                    ...
                    children: [
                      Transform.scale(
                        ...
                        child: AnimatedContainer(
                          ...
                          child: Icon(_typeIcon(cafe.type), color: color, size: 18),
                        ),
                      ),
                      CustomPaint(
  ...
  Widget _buildLegend() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text('Кластеры K-Means', style: TextStyle(
              color: Color(0xFFB47CFF), fontSize: 12,
